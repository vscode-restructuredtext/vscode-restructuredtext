...
export class EsbonioClient {
  ...
  async start(): Promise<void> {
    ...
    if (!this.client) {
      ...
      vscode.window
        .showErrorMessage(message, {title: 'Show Output'})
        .then(opt => {
  ...
  private async getStdioClient(): Promise<LanguageClient | undefined> {
    ...
    if (sourceFolder) {
      ...
      vscode.window.showInformationMessage(
        'debugpy is at port 5678. Connect and debug.'
      );
    } else {
      if (
        !(await this.python.checkPython(null, false)) ||
    ...
    if (
      startupModule.endsWith('.py') ||
      startupModule.includes('/') ||
      startupModule.includes('\\')
    ) {
  ...
  private configureHandlers() {
    this.client.onNotification('esbonio/buildStart', () => {
  ...
  private getLanguageClientOptions(
    ...
    const numJobs = config.get<number>('sphinx.numJobs');
    ...
    const documentSelector = [{scheme: 'file', language: 'restructuredtext'}];
    ...
    this.logger.debug(
      `LanguageClientOptions: ${JSON.stringify(clientOptions)}`
    );
