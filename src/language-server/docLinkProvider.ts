...
export class DocumentLinkProvider implements vscode.DocumentLinkProvider {
    ...
    public provideDocumentLinks(
        ...
        for (const directive of directives) {
            result = result.concat(
                this._findDirectiveLinks(document, directive, token)
            );
        }
        return result;
    }
    ...
    public async resolveDocumentLink(
        ...
        const type =
            link.tooltip.indexOf('document') > -1 ? 'doc' : 'directive';
    ...
    private _findDocLinks(
        ...
        for (const docRole of docRoles) {
            ...
            docRoleOffsetStart = docText.indexOf(
                docRole,
                docRoleOffsetStart + 1
            );
    ...
    private async _findTargetUri(
        ...
        const file: string = await this.client.sendRequest(
