...
export class RSTContentProvider {
    ...
    public async provideTextDocumentContent(
        ...
        const csp = this.getCspForResource(
            sourceUri,
            nonce,
            useSphinx,
            webview
        ...
        if (useSphinx) {
            ...
            const parsedDoc = body
                ...
                .join('\n');
            const newHead = parsedDoc.replace(
                '</head>',
                `
		...
		<base href="${webview.asWebviewUri(rstDocument.uri)}">
            ...
            this.logger.log(
                '[preview] Document line count: ' +
                    rstDocument.lineCount +
                    '; element count: ' +
                    elementCount
            );
            if (rstDocument.lineCount < elementCount) {
                this.logger.log(
                    '[preview] WARN: documentl line count is less than element count.'
                );
            }
            return newAll;
        } else {
                ...
                .map((l, i) =>
                    l.replace(
                        ...
                        (
                            ...
                            typeof p5 !== 'string'
                                ? `<${p1} class="code-line" data-line="${
                                      i + 1
                                  }" ${p2}`
                                : `<${p1} ${p3} class="${p5} code-line" data-line="${
                                      i + 1
                                  }" ${p6}`
                    )
                )
    ...
    private getStyles(
        ...
        const baseStyles = config.baseStyles
            .map(
                href =>
                    `<link rel="stylesheet" type="text/css" href="${fix(
                        href
                    )}">`
            )
            .join('\n');
