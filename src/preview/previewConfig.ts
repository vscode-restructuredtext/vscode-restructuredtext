...
export class RSTPreviewConfiguration {
  ...
  private constructor(resource: vscode.Uri) {
    ...
    const rstConfig = vscode.workspace.getConfiguration(
      'restructuredtext',
      resource
    );
    const rstEditorConfig = vscode.workspace.getConfiguration(
      '[rst]',
      resource
    );
    ...
    this.scrollPreviewWithEditor = !!rstConfig.get<boolean>(
      'preview.scrollPreviewWithEditor',
      true
    );
    this.scrollEditorWithPreview = !!rstConfig.get<boolean>(
      'preview.scrollEditorWithPreview',
      true
    );
    this.lineBreaks = !!rstConfig.get<boolean>('preview.breaks', false);
    this.doubleClickToSwitchToEditor = !!rstConfig.get<boolean>(
      'preview.doubleClickToSwitchToEditor',
      true
    );
    this.rstEditorSelection = !!rstConfig.get<boolean>(
      'preview.markEditorSelection',
      true
    );

    this.fontFamily = rstConfig.get<string | undefined>(
      'preview.fontFamily',
      undefined
    );
    this.fontSize = Math.max(
      8,
      +rstConfig.get<number>('preview.fontSize', NaN)
    );
    this.lineHeight = Math.max(
      0.6,
      +rstConfig.get<number>('preview.lineHeight', NaN)
    );
