...
export default class SelectedConfigFileStatus {
    ...
    public async update() {
        const resource = this.configuration.getActiveResource();
        if (!this.config || this.config.workspaceRoot !== resource?.fsPath) {
    ...
    public async reset(resource: vscode.Uri) {
        ...
        resource = resource ?? this.configuration.getActiveResource();
    ...
    public async refreshConfig(
        ...
        const configDir = await this.selector.findConfDir(
            this.inReset,
            resource
        );
