...
async function _removeDecoChar(decoType:("bold"|"italic"|"inlineRaw")):Promise<boolean> {
    function _removeDecoCharOneLine(line:string, startCharacter:number, endCharacter:number) {
        ...
        let removedDecoCharsInfo:(string|RemovedDecoCharInfo)[] = [];
        ...
        do {
            ...
            for (let i = 0; i < frontDecoText.length; i++) {
                removedDecoCharsInfo.push(
                    {removedChar: frontDecoText[i], position: "front"}
                );
            }
            ...
            for (let i = 0; i < backDecoText.length; i++) {
                removedDecoCharsInfo.push(
                    {removedChar: backDecoText[i], position: "back"}
                );
            }
        ...
        // Analyze Forward
        let hasRemovedDecoChar:boolean = false;
    ...
    let _removed:boolean = false;

    const editor = vscode.window.activeTextEditor;
    if (!editor) { return _removed }
